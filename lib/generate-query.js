...
function getRandomFields(fields, config, schema, depth) {
    ...
    if (results.length === 0) {
        ...
        if (!nextIsLeaf) {
            ...
            throw new Error(`Cannot pick field from: ${fields
...
function getArgsAndVars(allArgs, nodeName, fieldName, config, schema, providedValues) {
    ...
    if (config.providerMap) {
        ...
        try {
            ...
            const typeFieldProviderValue = provide_variables_1.getProviderValue(typeFieldName, config, providedValues);
            ...
            requiredArguments.forEach((arg) => {
                ...
                if (provide_variables_1.isEnumType(argType)) {
                    ...
                    try {
                        const typeFieldArgumentProviderValue = provide_variables_1.getProviderValue(varName, config, Object.assign(Object.assign({}, variableValues), providedValues), argType);
                        ...
                        if (!variableValues[varName]) {
                            throw new Error(`${e} Consider applying wildcard provider with "*__*" or "*__*__*"`);
                        }
                    }
                }
            });
