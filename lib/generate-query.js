...
function getArgsAndVars(allArgs, nodeName, fieldName, config, schema, providedValues) {
    ...
    if (config.providerMap) {
        ...
        let providedVariableValues = provide_variables_1.getProviderValue(typeFieldName, config, providedValues);
        ...
        requiredArguments.forEach(arg => {
            ...
            if (provide_variables_1.isEnumType(argType)) {
                ...
                const providedValue = provide_variables_1.getProviderValue(varName, config, Object.assign(Object.assign({}, variableValues), providedValues), argType);
                if (providedValue) {
    ...
    selections.forEach((selectionNode) => {
        if (selectionNode.kind === graphql_1.Kind.FIELD) {
            ...
            if (fieldName in aliasIndexes) {
                cleanselections.push(Object.assign(Object.assign({}, selectionNode), { alias: {
                        kind: graphql_1.Kind.NAME,
            ...
            selectionNode.selectionSet.selections.forEach((fragmentSelectionNode) => {
                if (fragmentSelectionNode.kind === graphql_1.Kind.FIELD) {
                    ...
                    if (fieldName in aliasIndexes) {
                        cleanFragmentSelections.push(Object.assign(Object.assign({}, fragmentSelectionNode), { alias: {
                                kind: graphql_1.Kind.NAME,
                                value: `${fieldName}${aliasIndexes[fieldName]++}`
                            } }));
                    }
...
function generateRandomMutation(schema, config = {}) {
    const finalConfig = Object.assign(Object.assign(Object.assign({}, DEFAULT_CONFIG), config), { seed: typeof config.seed !== 'undefined'
            ? config.seed
            : Math.random(), nodeFactor: 1, typeCount: 0, resolveCount: 0 });
