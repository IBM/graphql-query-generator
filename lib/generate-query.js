    ...
    return field.arguments.find((arg) => {
        return slicingArguments.find((slicingArgument) => {
            ...
            if (tokenizedPath.length < 1)
                return false;
            else if (arg.name.value !== tokenizedPath[0])
                return false;
            return true;
        });
    });
    ...
    requiredArguments.forEach((arg) => {
        ...
        if (typeof variableValues[varName] === 'undefined') {
            if (provide_variables_1.isEnumType(argType)) {
                ...
                const listSizeDirective = getListSizeDirective(field);
                if (listSizeDirective) {
                    const listSizeDirectiveDefaultValue = getListSizeDirectiveDefaultValue(listSizeDirective, arg.type, config, schema);
                    if (typeof listSizeDirectiveDefaultValue !== 'object')
                        variableValues[varName] = listSizeDirectiveDefaultValue;
                    else
                        variableValues[varName] = merge_1.default.recursive(listSizeDirectiveDefaultValue, variableValues[varName]);
                }
            }
...
function getListSizeDirectiveDefaultValue(listSizeDirective, typeNode, config, schema) {
    ...
    if (!slicingArgumentsArg)
        return undefined;
    let slicingArgumentsPaths = slicingArgumentsArg.value.values.map((value) => {
        if (value.kind === 'StringValue')
            return value.value;
    });
    const requireOneSlicingArg = getRequireOneSlicingArgument(listSizeDirective);
    if (!requireOneSlicingArg ||
        requireOneSlicingArg.value.value === true) {
        slicingArgumentsPaths = slicingArgumentsPaths.slice(0, 1);
    }
    const slicingArgumentsTokenizedPaths = slicingArgumentsPaths.map((value) => value.split('.'));
    if (slicingArgumentsPaths.length === 1 &&
    ...
    return slicingArgumentsTokenizedPaths.reduce((res, tokenizedPath) => {
        return merge_1.default.recursive(res, getListSizeDirectiveDefaultValueHelper(tokenizedPath, typeNode, config, schema));
    }, {});
}
function getListSizeDirectiveDefaultValueHelper(tokenizedPath, typeNode, config, schema) {
    const obj = {};
    if (tokenizedPath.length < 1)
        throw new Error('bad token path');
    ...
    if (namedType.kind === 'NamedType') {
        ...
        if (type.astNode.kind === 'InputObjectTypeDefinition') {
            ...
            obj[tokenizedPath[1]] = getListSizeDirectiveDefaultValueHelper(tokenizedPath.slice(1), nextType.type, config, schema);
